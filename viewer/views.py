...
def create_map():
  ...
  kml_file = os.path.join(settings.BASE_DIR, 'Bariflolabs-to-place.kml')
  geojson_features = []
...
def run_splat(transmitter_name, receiver_name):
    transmitter_qth = os.path.join(settings.BASE_DIR, f'{transmitter_name}.qth')
    receiver_qth = os.path.join(settings.BASE_DIR, f'{receiver_name}.qth')
    sdf_file = os.path.join(settings.BASE_DIR, '20x21x274x275.sdf')
    kml_output = os.path.join(settings.BASE_DIR, f'{transmitter_name}-to-{receiver_name}.kml')
    lrp_file = os.path.join(settings.BASE_DIR, f'{transmitter_name}.lrp')
...
def transmitter_data(request):
    if request.method == 'POST':
        try:
            ...
            return JsonResponse({"message": "Transmitter data saved successfully", "file_path": file_path})
        except Exception as e:
...
def receiver_data(request):
    if request.method == 'POST':
        try:
            ...
            return JsonResponse({"message": "Receiver data saved successfully", "file_path": file_path})
...
@csrf_exempt
def run_splat_endpoint(request):
    if request.method == 'POST':
        try:
            jsondata = JSONParser().parse(request)
            transmitter_name = jsondata.get("transmitter_name")
            ...
            if not all([transmitter_name, receiver_name]):
                return JsonResponse({"error": "missing parameter"})

            splat_success = run_splat(transmitter_name, receiver_name)
            if splat_success:
                return JsonResponse({"message": "SPLAT! run successfully"})
            else:
                return JsonResponse({"message": "SPLAT! run failed"})
        except Exception as e:
            return JsonResponse({"error": str(e)})
    else:
        return JsonResponse({"error": "invalid request method"})